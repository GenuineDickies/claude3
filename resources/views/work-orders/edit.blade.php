...
<div class="max-w-4xl mx-auto space-y-6" x-data="workOrderEditForm()">
    ...
    <div class="flex items-center gap-2 text-sm text-gray-500">
        ...
        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
        <a href="{{ route('work-orders.show', [$serviceRequest, $workOrder]) }}" class="hover:text-blue-600">{{ $workOrder->work_order_number }}</a>
        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
    ...
    <div class="bg-linear-to-r from-amber-50 to-white rounded-lg shadow-sm p-6 border-l-4 border-amber-500">
        <div class="flex justify-between items-start">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Edit {{ $workOrder->work_order_number }}</h1>
            ...
            @endphp
            <span class="px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wide {{ $statusColors[$workOrder->status] ?? 'bg-gray-100 text-gray-600' }}">
    ...
    <form action="{{ route('work-orders.update', [$serviceRequest, $workOrder]) }}" method="POST" @submit="prepareSubmit">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 border border-gray-200 space-y-4">
            <h2 class="text-lg font-semibold text-gray-800">Work Order Details</h2>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="2" placeholder="Description of work to be performed..."
                          class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('description', $workOrder->description) }}</textarea>
            </div>

            <div>
                <label for="technician_notes" class="block text-sm font-medium text-gray-700 mb-1">Technician Notes</label>
                <textarea id="technician_notes" name="technician_notes" rows="2" placeholder="Notes from technician..."
                          class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('technician_notes', $workOrder->technician_notes) }}</textarea>
            </div>
        </div>
        ...
        {{-- Add from Catalog --}}
        <div class="bg-white rounded-lg shadow-sm p-6 border border-gray-200 mt-6" x-data="{ search: '' }">
                            ...
                            @foreach($category->items as $item)
                                <div class="flex items-center justify-between py-2"
                                    ...
                                    <button type="button"
                                            ...
                                            class="shrink-0 text-blue-600 hover:text-blue-800 hover:bg-blue-50 rounded-sm p-1.5 transition" title="Add to work order">
        ...
        <div class="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-200 mt-6">
            ...
            {{-- Totals --}}
            <div class="border-t-2 border-gray-200 bg-amber-50/60 px-6 py-4" x-show="items.length > 0">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 border border-gray-200 mt-6">
            ...
            <textarea id="notes" name="notes" rows="3" placeholder="Optional notes..."
                      class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('notes', $workOrder->notes) }}</textarea>
        </div>
        ...
        <div class="flex justify-between items-center mt-6">
            <a href="{{ route('work-orders.show', [$serviceRequest, $workOrder]) }}"
               class="text-sm text-gray-500 hover:text-blue-600 underline">&larr; Back to {{ $workOrder->work_order_number }}</a>
