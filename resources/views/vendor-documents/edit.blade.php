...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <h1 class="text-2xl font-bold text-white">Edit Vendor Document</h1>

    <form method="POST" action="{{ route('vendor-documents.update', $doc) }}" class="space-y-6" x-data="vendorDocForm()">
        @csrf
        @method('PUT')

        {{-- Document header --}}
        <div class="surface-1 p-6">
            <h2 class="text-lg font-semibold text-gray-300 mb-4">Document Info</h2>
                ...
                <div>
                    ...
                    <select name="vendor_id" id="vendor_id"
                        ...
                        @foreach ($vendors as $v)
                            <option value="{{ $v->id }}" {{ old('vendor_id', $doc->vendor_id) == $v->id ? 'selected' : '' }}>{{ $v->name }}</option>
                        @endforeach
                    </select>
                    @error('vendor_id') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="document_type" class="block text-sm font-medium text-gray-300 mb-1">Type <span class="text-red-500">*</span></label>
                    <select name="document_type" id="document_type" x-model="docType"
                            class="w-full rounded-md border-white/10 shadow-xs text-sm input-crystal" required>
                        @foreach (\App\Models\VendorDocument::TYPES as $key => $label)
                            <option value="{{ $key }}" {{ old('document_type', $doc->document_type) === $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label for="document_date" class="block text-sm font-medium text-gray-300 mb-1">Date <span class="text-red-500">*</span></label>
                    <input type="date" name="document_date" id="document_date"
                           value="{{ old('document_date', $doc->document_date->format('Y-m-d')) }}"
                           class="w-full rounded-md border-white/10 shadow-xs text-sm input-crystal" required>
                </div>
                <div>
                    <label for="vendor_document_number" class="block text-sm font-medium text-gray-300 mb-1">Vendor Doc #</label>
                    <input type="text" name="vendor_document_number" id="vendor_document_number"
                           value="{{ old('vendor_document_number', $doc->vendor_document_number) }}"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="flex items-center gap-3">
                        ...
                        <span class="text-sm font-medium text-gray-300">Paid</span>
                    </label>
                </div>
                <div>
                    ...
                    <select name="payment_method" id="payment_method"
                        ...
                        @foreach (\App\Models\VendorDocument::PAYMENT_METHODS as $key => $label)
                            <option value="{{ $key }}" {{ old('payment_method', $doc->payment_method) === $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div class="surface-1 p-6">
                            ...
                            <tr class="border-b border-white/10">
                                <td class="py-2 pr-2">
                                    <select :name="'lines[' + index + '][line_type]'" x-model="line.line_type"
                                            class="w-full rounded-md border-white/10 shadow-xs text-xs input-crystal" required>
                                ...
                                <td class="py-2 pr-2">
                                    <div class="relative">
                                        ...
                                        <input type="number" :name="'lines[' + index + '][unit_cost]'" x-model.number="line.unit_cost"
                                               ...
                                               class="w-full pl-5 rounded-md border-white/10 shadow-xs text-sm input-crystal" required>
                                    </div>
                                </td>
                                <td class="py-2 pr-2">
                                    <div class="relative">
                                        ...
                                        <input type="number" :name="'lines[' + index + '][core_amount]'" x-model.number="line.core_amount"
                                               step="0.01" min="0"
                                               class="w-full pl-5 rounded-md border-white/10 shadow-xs text-sm input-crystal">
                                    </div>
                                </td>
                                <td class="py-2 pr-2">
                                    <select :name="'lines[' + index + '][expense_account_id]'" x-model="line.expense_account_id"
                                            class="w-full rounded-md border-white/10 shadow-xs text-xs input-crystal">
            ...
            <button type="button" @click="addLine()"
                    class="mt-3 text-sm text-cyan-400 hover:text-cyan-300 underline">+ Add Line</button>
        ...
        <div class="surface-1 p-6">
            ...
            <textarea name="notes" id="notes" rows="3"
                      class="w-full rounded-md border-white/10 shadow-xs text-sm input-crystal">{{ old('notes', $doc->notes) }}</textarea>
        </div>
...
function vendorDocForm() {
    const existingLines = @json($doc->lines->map(fn ($l) => [
        'line_type' => $l->line_type,
        'description' => $l->description,
        'part_id' => $l->part_id ?? '',
        'qty' => (float) $l->qty,
        'unit_cost' => (float) $l->unit_cost,
        'core_amount' => (float) $l->core_amount,
        'taxable' => (bool) $l->taxable,
        'expense_account_id' => $l->expense_account_id ?? '',
    ])->values());

    return {
        docType: '{{ old('document_type', $doc->document_type) }}',
        isPaid: {{ old('is_paid', $doc->is_paid) ? 'true' : 'false' }},
        lines: existingLines.length ? existingLines : [{ line_type: 'part', description: '', part_id: '', qty: 1, unit_cost: 0, core_amount: 0, taxable: false, expense_account_id: '' }],
        subtotal: 0,
        total: 0,
