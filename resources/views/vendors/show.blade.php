...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        ...
        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
            ...
            <div>
                ...
                <dd class="font-medium text-gray-900 mt-0.5">{{ $vendor->phone }}</dd>
            </div>
            @endif
            @if ($vendor->email)
            <div>
                <dt class="text-gray-500">Email</dt>
                <dd class="font-medium text-gray-900 mt-0.5">{{ $vendor->email }}</dd>
            </div>
            @endif
            @if ($vendor->fullAddress())
            <div class="sm:col-span-2">
                <dt class="text-gray-500">Address</dt>
                <dd class="font-medium text-gray-900 mt-0.5">{{ $vendor->fullAddress() }}</dd>
            </div>
            @endif
            @if ($vendor->account_number)
            <div>
                <dt class="text-gray-500">Account Number</dt>
                <dd class="font-medium text-gray-900 mt-0.5 font-mono">{{ $vendor->account_number }}</dd>
            </div>
            @endif
            @if ($vendor->payment_terms)
            <div>
                <dt class="text-gray-500">Payment Terms</dt>
                <dd class="font-medium text-gray-900 mt-0.5">{{ $vendor->payment_terms }}</dd>
            </div>
            @endif
            @if ($vendor->defaultExpenseAccount)
            <div>
                <dt class="text-gray-500">Default Expense Account</dt>
                <dd class="font-medium text-gray-900 mt-0.5">{{ $vendor->defaultExpenseAccount->code }} – {{ $vendor->defaultExpenseAccount->name }}</dd>
            </div>
            @endif
        </dl>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-lg font-semibold text-gray-700">Recent Documents</h2>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach ($vendor->documents as $doc)
                            <tr class="hover:bg-gray-50">
                                <td class="px-3 py-2 text-gray-600">{{ $doc->document_date->format('M j, Y') }}</td>
                                <td class="px-3 py-2">
                                    <span class="inline-block px-2 py-0.5 rounded-full text-xs font-semibold {{ $doc->isReceipt() ? 'bg-yellow-100 text-yellow-700' : 'bg-blue-100 text-blue-700' }}">
                                        {{ $doc->typeLabel() }}
                                    </span>
                                </td>
