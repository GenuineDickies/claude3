{{-- Sub-navigation bar shared by all document views --}}
<nav class="surface-1 border border-white/10 px-1.5 py-1.5 flex items-center gap-1 text-sm">
    <a href="{{ route('inbox.index') }}"
       class="flex items-center gap-1.5 px-3 py-1.5 rounded-md font-medium transition-colors
              {{ request()->routeIs('inbox.*') ? 'bg-cyan-500/10 text-cyan-400' : 'text-gray-500 hover:text-gray-300 hover:bg-white/5' }}">
    ...
    <a href="{{ route('transaction-imports.index') }}"
       class="flex items-center gap-1.5 px-3 py-1.5 rounded-md font-medium transition-colors
              {{ request()->routeIs('transaction-imports.*') ? 'bg-cyan-500/10 text-cyan-400' : 'text-gray-500 hover:text-gray-300 hover:bg-white/5' }}">
    ...
    <a href="{{ route('accounting.import-accounts') }}"
       class="flex items-center gap-1.5 px-3 py-1.5 rounded-md font-medium transition-colors
              {{ request()->routeIs('accounting.import-accounts') ? 'bg-cyan-500/10 text-cyan-400' : 'text-gray-500 hover:text-gray-300 hover:bg-white/5' }}">
    ...
    {{-- Breadcrumb context (shown when deeper in) --}}
    @if (isset($breadcrumb))
        <span class="text-gray-300 mx-1">/</span>
        <span class="text-gray-300 font-medium truncate">{{ $breadcrumb }}</span>
    @endif
</nav>
