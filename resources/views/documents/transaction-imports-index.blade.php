...
<div class="max-w-6xl mx-auto space-y-5">
    ...
    {{-- Header --}}
    <div>
        <h1 class="text-2xl font-bold text-gray-800">Transaction Imports</h1>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-5 gap-4">
        ...
        <div class="bg-white rounded-lg border p-4 text-center">
            <div class="text-2xl font-bold text-gray-800">{{ $stats['total_rows'] }}</div>
    ...
    @if ($documents->isEmpty())
        <div class="bg-white rounded-lg border p-8 text-center text-gray-500">
            ...
            <p class="text-sm mt-2">Upload spreadsheets via the <a href="{{ route('inbox.index') }}" class="text-blue-600 hover:underline font-medium">Document Inbox</a> to get started.</p>
        </div>
    @else
        <div class="space-y-3">
            @foreach ($documents as $doc)
                ...
                <a href="{{ route('transaction-imports.show', $doc) }}"
                   class="block bg-white rounded-lg border hover:shadow-md transition-shadow {{ $allDone ? 'border-green-200' : '' }}">
                    <div class="p-4">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3 min-w-0">
                                {{-- Spreadsheet icon --}}
                                <div class="w-10 h-10 rounded-lg {{ $allDone ? 'bg-green-50' : 'bg-amber-50' }} flex items-center justify-center shrink-0">
                                    @if ($allDone)
                                ...
                                <div class="min-w-0">
                                    <div class="font-medium text-gray-800 truncate">{{ $doc->original_filename }}</div>
