...
<div class="max-w-7xl mx-auto space-y-5">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Review Transactions</h1>
        ...
        @if ($stats['draft'] > 0)
            <div class="flex gap-2 shrink-0">
                ...
                <form method="POST" action="{{ route('transaction-imports.bulk-reject', $document) }}"
                    ...
                    @csrf
                    <button type="submit" class="inline-flex items-center gap-1.5 px-4 py-2 bg-red-50 text-red-700 text-sm font-medium rounded-lg hover:bg-red-100 border border-red-200 transition-colors">
    ...
    @endif
    @if (session('info'))
        <div class="bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded-lg text-sm">{{ session('info') }}</div>
    @endif
    ...
    <div class="bg-white rounded-lg border p-4">
        <div class="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm">
            ...
            <div class="flex items-center gap-2">
                ...
                <span class="font-bold text-green-600">{{ $stats['accepted'] }}</span>
            </div>
            <div class="flex items-center gap-2">
                <span class="w-2 h-2 rounded-full bg-red-400"></span>
                <span class="text-gray-500">Rejected:</span>
                <span class="font-bold text-red-600">{{ $stats['rejected'] }}</span>
            </div>
            <div class="flex items-center gap-2 ml-auto">
                <span class="text-gray-500">Pending Total:</span>
                <span class="font-bold text-gray-800">${{ number_format($stats['sum'], 2) }}</span>
            </div>
        </div>
        @php
        ...
        <div class="mt-3">
            <div class="w-full bg-gray-100 rounded-full h-2 flex overflow-hidden">
                @if ($acceptPct > 0)
                    <div class="bg-green-500 h-full" style="width: {{ $acceptPct }}%"></div>
                @endif
                @if ($rejectPct > 0)
    ...
    @endphp
    <div x-data="{ summaryOpen: false }" class="bg-white rounded-lg border">
        <button @click="summaryOpen = !summaryOpen" class="w-full flex items-center justify-between px-4 py-3 text-left hover:bg-gray-50 transition-colors">
            <div class="flex items-center gap-3">
                <h3 class="text-sm font-semibold text-gray-700">Accounting Summary</h3>
                <div class="flex items-center gap-3 text-xs text-gray-500">
                    @if (isset($typeBreakdown['expense']))
                        <span class="text-red-600">${{ number_format($expenseTotal, 2) }} expense</span>
                    @endif
                    @if (isset($typeBreakdown['income']))
                        <span class="text-green-600">${{ number_format($incomeTotal, 2) }} income</span>
                    @endif
                    ...
                    @if (isset($typeBreakdown['income']))
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm text-gray-700">Income ({{ $typeBreakdown['income']['count'] }})</span>
                            </div>
                            <span class="text-sm font-medium text-green-600">${{ number_format($incomeTotal, 2) }}</span>
                        </div>
                    @endif
                    @if (isset($typeBreakdown['expense']))
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm text-gray-700">Expenses ({{ $typeBreakdown['expense']['count'] }})</span>
                            </div>
                            <span class="text-sm font-medium text-red-600">${{ number_format($expenseTotal, 2) }}</span>
                        </div>
                    @endif
                    @if (isset($typeBreakdown['transfer']))
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm text-gray-700">Transfers ({{ $typeBreakdown['transfer']['count'] }})</span>
                            </div>
                            <span class="text-sm font-medium text-blue-600">${{ number_format($transferTotal, 2) }}</span>
                        </div>
                    @endif
                </div>
                    ...
                    <div class="space-y-1.5 max-h-48 overflow-y-auto">
                        @foreach ($categoryBreakdown as $catKey => $cat)
                            <div class="flex items-center justify-between text-sm">
                                <span class="text-gray-600">{{ $cat['label'] }} <span class="text-gray-400">({{ $cat['count'] }})</span></span>
    ...
    <div class="bg-white rounded-lg border overflow-hidden">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($imports as $row)
                        <tr class="{{ $row->status === 'accepted' ? 'bg-green-50/50' : ($row->status === 'rejected' ? 'bg-red-50/50 opacity-60' : '') }}">
                            ...
                            <td class="px-3 py-2.5 whitespace-nowrap text-sm text-gray-900">{{ $row->transaction_date?->format('m/d/Y') ?? '—' }}</td>
                            <td class="px-3 py-2.5 text-sm text-gray-900 max-w-xs truncate" title="{{ $row->description }}">{{ Str::limit($row->description, 45) }}</td>
                            <td class="px-3 py-2.5 text-sm text-gray-700 whitespace-nowrap">{{ Str::limit($row->vendor ?? '—', 20) }}</td>
                            <td class="px-3 py-2.5 text-sm text-gray-700 whitespace-nowrap">
                                {{ $row->categoryLabel() }}
                                @if ($row->type === 'income')
                                    <span class="text-[10px] text-green-600 font-medium ml-1">IN</span>
                                @elseif ($row->type === 'transfer')
                                    <span class="text-[10px] text-blue-600 font-medium ml-1">TFR</span>
                                @endif
                            </td>
                            <td class="px-3 py-2.5 whitespace-nowrap text-sm font-medium text-right {{ $row->type === 'expense' ? 'text-red-600' : 'text-green-600' }}">
                                ${{ number_format($row->amount, 2) }}
                            </td>
                            <td class="px-3 py-2.5 text-sm text-gray-700 whitespace-nowrap">{{ $row->account_code ?? '—' }}</td>
                            <td class="px-3 py-2.5 whitespace-nowrap text-center">
                                @if ($row->status === 'draft')
                                    <div class="flex items-center justify-center gap-1">
                                        <button onclick="openEditModal({{ $row->id }})"
                                                class="p-1.5 text-blue-600 hover:bg-blue-50 rounded" title="Edit & Accept">
                                        ...
                                        <form method="POST" action="{{ route('transaction-imports.accept', $row) }}" class="inline">
                                            @csrf
                                            <button type="submit" class="p-1.5 text-green-600 hover:bg-green-50 rounded" title="Accept">
                                        ...
                                        <form method="POST" action="{{ route('transaction-imports.reject', $row) }}" class="inline">
                                            @csrf
                                            <button type="submit" class="p-1.5 text-red-600 hover:bg-red-50 rounded" title="Reject">
                                ...
                                @elseif ($row->status === 'accepted')
                                    <a href="{{ route('expenses.show', $row->created_expense_id) }}" class="text-xs text-blue-600 hover:underline">View Expense</a>
                                @else
        ...
        <div class="lg:hidden divide-y divide-gray-100">
            @forelse ($imports as $row)
                <div class="p-4 {{ $row->status === 'accepted' ? 'bg-green-50/50' : ($row->status === 'rejected' ? 'bg-red-50/30 opacity-70' : '') }}">
                    <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="text-sm font-medium text-gray-900 mt-1.5">{{ Str::limit($row->description, 60) }}</p>
                        ...
                        <div class="text-right shrink-0">
                            <p class="text-sm font-bold {{ $row->type === 'expense' ? 'text-red-600' : 'text-green-600' }}">
                                ${{ number_format($row->amount, 2) }}
                            </p>
                    ...
                    @if ($row->status === 'draft')
                        <div class="flex items-center gap-2 mt-3 pt-2 border-t border-gray-100">
                            <button onclick="openEditModal({{ $row->id }})"
                                    class="flex-1 inline-flex items-center justify-center gap-1 px-3 py-1.5 text-xs font-medium text-blue-700 bg-blue-50 rounded-md hover:bg-blue-100 transition-colors">
                            ...
                            <form method="POST" action="{{ route('transaction-imports.accept', $row) }}" class="flex-1">
                                @csrf
                                <button type="submit" class="w-full inline-flex items-center justify-center gap-1 px-3 py-1.5 text-xs font-medium text-green-700 bg-green-50 rounded-md hover:bg-green-100 transition-colors">
                    ...
                    @elseif ($row->status === 'accepted')
                        <div class="mt-2">
                            <a href="{{ route('expenses.show', $row->created_expense_id) }}" class="text-xs text-blue-600 hover:underline">View Expense &rarr;</a>
                        </div>
                    @endif
                </div>
            @empty
    ...
    @if ($document->ai_summary)
        <div class="bg-white rounded-lg border p-4">
            <h3 class="text-sm font-semibold text-gray-700 mb-2">AI Document Summary</h3>
            <p class="text-sm text-gray-600">{{ $document->ai_summary }}</p>
        </div>
    @endif
</div>
...
<div id="editModal" class="fixed inset-0 z-50 hidden">
    ...
    <div class="absolute inset-0 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto relative">
            <div class="p-6">
                <h3 class="text-lg font-semibold text-gray-800 mb-4">Edit Transaction Before Accepting</h3>
