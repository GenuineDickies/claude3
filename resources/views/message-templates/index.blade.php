...
<div class="max-w-6xl mx-auto">
    ...
    <div x-data="{ open: false }" class="bg-white rounded-lg shadow-xs mb-6">
        <button @click="open = !open" class="w-full flex items-center justify-between px-6 py-4 text-left">
            <div>
                <h2 class="text-base font-semibold text-gray-900">Available Variables</h2>
                <p class="text-xs text-gray-500 mt-0.5">Use <code class="bg-gray-100 px-1 py-0.5 rounded-sm text-xs">@{{ variable_name }}</code> in template bodies to insert dynamic data.</p>
            </div>
        ...
        <div x-show="open" x-collapse class="px-6 pb-5">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                ...
                @foreach($vars as $key => $def)
                    <div class="flex items-center gap-2 p-2 bg-gray-50 rounded-sm">
                        <code class="text-xs bg-blue-50 text-blue-700 px-1.5 py-0.5 rounded-sm font-mono whitespace-nowrap">@{{ {{ e($key) }} }}</code>
                        <span class="text-xs text-gray-600 truncate">{{ $def['label'] }}</span>
                    </div>
                @endforeach
            </div>
        </div>
                        ...
                        <div class="flex items-start justify-between gap-4 px-6 py-4">
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2">
                                    <span class="inline-flex items-center justify-center w-8 h-5 rounded-sm bg-gray-200 text-[10px] font-bold text-gray-600 font-mono shrink-0" title="Template ID">{{ $template->id }}</span>
                                    <a href="{{ route('message-templates.show', $template) }}"
                                       class="text-sm font-medium text-gray-900 hover:text-blue-600">
                                        {{ $template->name }}
                                    </a>
                                    @if(!$template->is_active)
                                        <span class="inline-block px-1.5 py-0.5 text-xs font-medium bg-gray-100 text-gray-500 rounded-sm">Inactive</span>
                                    @endif
                                </div>
                                ...
                                @if($template->variables && count($template->variables))
                                    <div class="flex flex-wrap gap-1 mt-1.5">
                                        @foreach($template->variables as $var)
                                            <span class="inline-block text-[10px] bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded-sm font-mono">{{ $var }}</span>
                                        @endforeach
                                    </div>
                                @endif
                            </div>
                            <div class="flex items-center gap-2 shrink-0">
                                ...
                                <a href="{{ route('message-templates.show', $template) }}"
                                   class="text-sm text-gray-400 hover:text-green-600" title="Preview">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
