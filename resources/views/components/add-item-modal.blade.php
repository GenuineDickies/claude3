...
<div x-data="addItemModal()" class="relative">
    ...
    <div x-show="show" x-cloak class="fixed inset-0 z-50 overflow-y-auto px-4 py-6 sm:px-6">
        <div class="fixed inset-0 bg-slate-950/95 backdrop-blur-md transition-opacity" @click="close()"></div>
        ...
        <div x-show="show"
             ...
             class="relative mx-auto w-full max-w-3xl overflow-hidden rounded-3xl border border-cyan-500/30 bg-slate-950/95 shadow-2xl shadow-cyan-500/20">
            <div class="flex items-center justify-between gap-4 border-b border-white/20 px-6 py-5 bg-linear-to-r from-slate-900/80 to-slate-900/60">
                <div>
                    <p class="text-sm font-bold text-white">Add item to estimate</p>
                    <p class="text-sm text-gray-300">Search your catalog or create a blank item.</p>
                </div>
                <button type="button" @click="close()" class="rounded-full border border-white/20 bg-slate-900/80 p-2 text-gray-300 transition hover:border-white/40 hover:text-white hover:bg-slate-800">
            ...
            <div class="px-6 py-5 sm:px-8 sm:py-6">
                <div class="mb-5">
                    ...
                    <input id="add-item-search" x-ref="searchInput" x-model="search" type="search"
                           ...
                           class="w-full rounded-2xl border border-cyan-500/30 bg-slate-900/80 px-4 py-3 text-sm text-white placeholder:text-gray-400 focus:border-cyan-400 focus:outline-none focus:ring-2 focus:ring-cyan-500/50" />
                </div>

                <div class="space-y-5 max-h-[60vh] overflow-y-auto pr-1">
                    <template x-for="category in filteredCategories" :key="category.id">
                        <div class="rounded-3xl border border-white/20 bg-slate-950/80 p-4">
                            <div class="flex items-center justify-between gap-3 pb-3 border-b border-white/20">
                                <div>
                                    <p class="text-sm font-bold text-white" x-text="category.name"></p>
                                    <p class="text-xs text-gray-400" x-text="category.items.length + ' item' + (category.items.length === 1 ? '' : 's')"></p>
                                </div>
                            </div>

                            <template x-for="item in category.items.filter(matches)" :key="item.id">
                                <div class="mt-3 rounded-2xl border border-cyan-500/20 bg-slate-900/50 p-4 hover:border-cyan-500/60 hover:bg-slate-900/80 transition">
                                    <div class="flex items-start justify-between gap-4">
                                        <div class="min-w-0">
                                            <p class="text-sm font-bold text-white" x-text="item.name"></p>
                                            <p class="mt-1 text-sm text-gray-300" x-text="item.description || 'No description'"></p>
                                        </div>
