...
<nav aria-label="Main navigation" class="flex flex-col h-full w-64" style="background: linear-gradient(180deg, rgba(20, 27, 45, 0.96), rgba(12, 17, 31, 0.94)); backdrop-filter: blur(24px) saturate(150%); -webkit-backdrop-filter: blur(24px) saturate(150%); border-right: 1px solid rgba(255,255,255,0.1); box-shadow: 12px 0 36px rgba(0,0,0,0.28);">
    @php
    ...
    {{-- ── Brand ────────────────────────────────────────── --}}
    <div class="flex items-center h-16 px-5 shrink-0" style="border-bottom: 1px solid rgba(255,255,255,0.1);">
        <a href="{{ route('dashboard') }}" class="flex min-w-0 items-center gap-3 text-white">
            <span class="flex h-11 w-14 items-center justify-center overflow-hidden rounded-2xl bg-slate-950/70 ring-1 ring-white/10 shrink-0 px-2 py-1.5">
                @if(!empty($companyLogoUrl))
                    <img src="{{ $companyLogoUrl }}" alt="{{ $companyName }} logo" class="max-h-full max-w-full object-contain">
                @else
                    <span class="text-sm font-semibold tracking-[0.2em] text-cyan-100">{{ $brandInitials }}</span>
                @endif
            </span>
            <span class="truncate text-lg sm:text-xl font-bold tracking-tight">{{ $companyName }}</span>
        </a>
    </div>
    ...
    <div class="flex-1 overflow-y-auto px-3 py-3 sidebar-scroll">
        ...
        <a href="{{ route('dashboard') }}"
           class="sidebar-nav-item {{request()->routeIs('dashboard') ? 'sidebar-nav-item-active' : '' }}">
        ...
        <a href="{{ route('rapid-dispatch.create') }}"
           class="sidebar-nav-item {{request()->routeIs('rapid-dispatch.*') ? 'sidebar-nav-item-active' : '' }}">
            {{-- Heroicon: bolt --}}
        ...
        @if (Route::has('expenses.index'))
            <a href="{{ route('expenses.index') }}"
               class="sidebar-nav-item {{request()->routeIs('expenses.*') ? 'sidebar-nav-item-active' : '' }}">
                {{-- Heroicon: banknotes --}}
        ...
        <a href="{{ route('inbox.index') }}"
           class="sidebar-nav-item {{request()->routeIs('inbox.*') ? 'sidebar-nav-item-active' : '' }}">
        ...
        <a href="{{ route('transaction-imports.index') }}"
           class="sidebar-nav-item {{request()->routeIs('transaction-imports.*') ? 'sidebar-nav-item-active' : '' }}">
        ...
        <a href="{{ route('accounting.import-accounts') }}"
           class="sidebar-nav-item {{request()->routeIs('accounting.import-accounts') ? 'sidebar-nav-item-active' : '' }}">
            {{-- Heroicon: tag --}}
        ...
        <a href="{{ route('accounting.chart-of-accounts') }}"
           class="sidebar-nav-item {{request()->routeIs('accounting.chart-of-accounts') ? 'sidebar-nav-item-active' : '' }}">
            {{-- Heroicon: book-open --}}
        ...
        <a href="{{ route('accounting.journal') }}"
           class="sidebar-nav-item {{request()->routeIs('accounting.journal') ? 'sidebar-nav-item-active' : '' }}">
            {{-- Heroicon: document-text --}}
            <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z"/></svg>
            Journal Entries
        </a>
        ...
        <a href="{{ route('accounting.trial-balance') }}"
           class="sidebar-nav-item {{request()->routeIs('accounting.trial-balance') ? 'sidebar-nav-item-active' : '' }}">
            {{-- Heroicon: scale --}}
        ...
        <a href="{{ route('accounting.profit-loss') }}"
           class="sidebar-nav-item {{request()->routeIs('accounting.profit-loss') ? 'sidebar-nav-item-active' : '' }}">
        ...
        <a href="{{ route('accounting.balance-sheet') }}"
           class="sidebar-nav-item {{request()->routeIs('accounting.balance-sheet') ? 'sidebar-nav-item-active' : '' }}">
            {{-- Heroicon: calculator --}}
        ...
        <a href="{{ route('message-templates.index') }}"
           class="sidebar-nav-item {{request()->routeIs('message-templates.*') ? 'sidebar-nav-item-active' : '' }}">
        ...
        <a href="{{ route('settings.tax-rates') }}"
           class="sidebar-nav-item {{request()->routeIs('settings.tax-rates*') ? 'sidebar-nav-item-active' : '' }}">
        ...
        <a href="{{ route('settings.api-monitor.index') }}"
           class="sidebar-nav-item {{request()->routeIs('settings.api-monitor.*') ? 'sidebar-nav-item-active' : '' }}">
        ...
        @if (app()->environment('local', 'testing'))
            ...
            <a href="/test-location"
               class="sidebar-nav-item {{request()->is('test-location') ? 'sidebar-nav-item-active' : '' }}">
                {{-- Heroicon: map-pin --}}
    ...
    <div class="mt-auto px-3 py-3 shrink-0" style="border-top: 1px solid rgba(255,255,255,0.06);">
        <div class="px-3 mb-2">
        ...
        <a href="{{ route('profile.edit') }}"
           class="sidebar-nav-item {{request()->routeIs('profile.*') ? 'sidebar-nav-item-active' : '' }}">
