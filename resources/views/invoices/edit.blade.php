...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    {{-- Breadcrumb --}}
    <a href="{{ route('invoices.show', [$serviceRequest, $invoice]) }}" class="inline-flex items-center text-sm text-gray-500 hover:text-blue-600">
        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/></svg>
        Back to Invoice {{ $invoice->displayNumber() }}
    </a>

    <h1 class="text-2xl font-bold text-gray-900">Edit Invoice {{ $invoice->displayNumber() }}</h1>
    ...
    <form method="POST" action="{{ route('invoices.update', [$serviceRequest, $invoice]) }}" class="space-y-6" x-data="invoiceEditForm()">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            <h2 class="text-lg font-semibold text-gray-700 mb-4">Customer</h2>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label for="customer_name" class="block text-sm font-medium text-gray-700 mb-1">Name <span class="text-red-500">*</span></label>
        ...
        {{-- Vehicle & Service --}}
        <div class="bg-white rounded-lg shadow-sm p-6">
            <h2 class="text-lg font-semibold text-gray-700 mb-4">Service Details</h2>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label for="vehicle_description" class="block text-sm font-medium text-gray-700 mb-1">Vehicle</label>
                ...
                <div class="sm:col-span-2">
                    <label for="service_location" class="block text-sm font-medium text-gray-700 mb-1">Location</label>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <button type="button" @click="addItem()"
                    class="mt-3 text-sm text-blue-600 hover:text-blue-800 underline">+ Add Item</button>
            ...
            <div class="mt-4 border-t pt-4 space-y-2 max-w-xs ml-auto text-sm">
                ...
                <div class="flex justify-between items-center gap-2">
                    ...
                    <input type="number" name="tax_rate" x-model.number="taxRate" @input="recalculate()"
                           ...
                           class="w-20 rounded-md border-gray-300 shadow-xs text-sm text-right focus:border-blue-500 focus:ring-blue-500">
                </div>
                <div class="flex justify-between">
        ...
        {{-- Terms & Notes --}}
        <div class="bg-white rounded-lg shadow-sm p-6">
            <h2 class="text-lg font-semibold text-gray-700 mb-4">Terms & Notes</h2>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
            ...
            <div class="mt-4">
                ...
                <textarea name="notes" id="notes" rows="2"
                          class="w-full rounded-md border-gray-300 shadow-xs text-sm focus:border-blue-500 focus:ring-blue-500 resize-none">{{ old('notes', $invoice->notes) }}</textarea>
            </div>
        </div>
        ...
        <div class="flex gap-3">
            <button type="submit"
                    class="inline-flex items-center px-6 py-2 bg-blue-600 text-white text-sm font-semibold rounded-md hover:bg-blue-700 transition">
            ...
            <a href="{{ route('invoices.show', [$serviceRequest, $invoice]) }}"
               class="px-4 py-2 border border-gray-300 text-gray-700 text-sm font-medium rounded-md hover:bg-gray-50 transition">
                Cancel
            </a>
        </div>
    </form>
