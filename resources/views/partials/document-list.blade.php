...
<form method="POST" action="{{ $uploadUrl }}" enctype="multipart/form-data"
      class="flex flex-wrap items-end gap-3 mb-4 pb-4 border-b border-white/10">
    @csrf
    ...
    <button type="submit"
            class="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md  transition-colors">
        Upload
    </button>
</form>
        ...
        <li class="flex items-center justify-between py-2">
            <div class="flex items-center gap-3 min-w-0">
                ...
                <div class="min-w-0">
                    <a href="{{ route('documents.detail', $doc) }}"
                       class="text-sm font-medium text-cyan-400 hover:text-cyan-300 truncate block">{{ $doc->original_filename }}</a>
            ...
            <div class="flex items-center gap-2 shrink-0">
                <a href="{{ route('documents.show', $doc) }}" target="_blank"
                   class="text-gray-400 hover:text-gray-400 p-1" title="Download">
                ...
                <form method="POST" action="{{ route('documents.destroy', $doc) }}"
                    ...
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="text-red-400 hover:text-red-400 p-1" title="Delete">
