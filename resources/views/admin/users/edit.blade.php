        ...
        <form method="POST" action="{{ route('admin.users.update', $managedUser) }}" class="space-y-6"
            ...
            <div class="grid gap-6 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input id="phone" name="phone" type="tel" value="{{ old('phone', $managedUser->phone) }}" x-bind:required="requiresMobilePhone() && status === 'active'" class="w-full rounded-md border-white/10 text-sm shadow-sm input-crystal" placeholder="5551234567">
                    ...
                    <template x-if="requiresSmsConsent()">
                        <p class="mt-1 text-xs text-gray-500">
                            {{ $managedUser->hasSmsConsent() ? 'SMS consent is already on file for this user.' : 'The user must record their own SMS consent from their Profile page.' }}
                        </p>
                    </template>
                ...
                <div class="mt-3 grid gap-3 md:grid-cols-2">
                    @foreach ($roles as $role)
                        <label class="flex items-start gap-3 rounded-lg border border-white/10 p-3 hover:border-blue-300 hover:bg-cyan-500/10/40">
                            <input type="checkbox" name="role_ids[]" value="{{ $role->id }}" x-model="selectedRoleIds" @checked(in_array($role->id, old('role_ids', $managedUser->roles->pluck('id')->all()), true)) class="mt-1 rounded border-white/10 text-cyan-400 focus:ring-cyan-500">
                            <span>
