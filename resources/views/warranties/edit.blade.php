...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    {{-- Breadcrumb --}}
    <a href="{{ route('warranties.show', [$serviceRequest, $warranty]) }}" class="inline-flex items-center text-sm text-gray-500 hover:text-blue-600">
        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/></svg>
        {{ $warranty->part_name }}
    </a>

    <h1 class="text-2xl font-bold text-gray-900">Edit Warranty</h1>
    ...
    <form method="POST" action="{{ route('warranties.update', [$serviceRequest, $warranty]) }}" class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <textarea name="notes" id="notes" rows="3"
                      class="w-full rounded-md border-gray-300 shadow-xs text-sm focus:border-blue-500 focus:ring-blue-500">{{ old('notes', $warranty->notes) }}</textarea>
