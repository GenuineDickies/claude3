...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="surface-1 p-6">
        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-white">{{ $warranty->part_name }}</h1>
    ...
    {{-- Warranty details --}}
    <div class="surface-1 p-6">
        <h2 class="text-lg font-semibold text-gray-300 mb-4">Warranty Details</h2>
        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
            <div>
                <dt class="text-gray-500">Install Date</dt>
                <dd class="font-medium text-white mt-0.5">{{ $warranty->install_date->format('M j, Y') }}</dd>
            </div>
            <div>
                <dt class="text-gray-500">Duration</dt>
                <dd class="font-medium text-white mt-0.5">{{ $warranty->warranty_months }} month{{ $warranty->warranty_months !== 1 ? 's' : '' }}</dd>
            </div>
            <div>
                <dt class="text-gray-500">Expires</dt>
                <dd class="font-medium text-white mt-0.5">{{ $warranty->warranty_expires_at->format('M j, Y') }}</dd>
            </div>
            <div>
                <dt class="text-gray-500">Customer</dt>
                <dd class="font-medium text-white mt-0.5">
    ...
    <div class="surface-1 p-6">
        <h2 class="text-lg font-semibold text-gray-300 mb-4">Vendor</h2>
        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
            @if ($warranty->vendor_name)
            <div>
                <dt class="text-gray-500">Name</dt>
                <dd class="font-medium text-white mt-0.5">{{ $warranty->vendor_name }}</dd>
            </div>
            @endif
            @if ($warranty->vendor_phone)
            <div>
                <dt class="text-gray-500">Phone</dt>
                <dd class="font-medium text-white mt-0.5 font-mono">{{ $warranty->vendor_phone }}</dd>
            </div>
            @endif
            @if ($warranty->vendor_invoice_number)
            <div>
                <dt class="text-gray-500">Invoice #</dt>
                <dd class="font-medium text-white mt-0.5 font-mono">{{ $warranty->vendor_invoice_number }}</dd>
            </div>
            @endif
        </dl>
    </div>
    ...
    <div class="surface-1 p-6">
        ...
        <p class="text-sm text-gray-300 whitespace-pre-line">{{ $warranty->notes }}</p>
    </div>
    @endif
