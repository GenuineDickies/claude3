...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    {{-- Breadcrumb --}}
    <a href="{{ route('expenses.index') }}" class="inline-flex items-center text-sm text-gray-500 hover:text-blue-600">
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">{{ $expense->expense_number }}</h1>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        ...
        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
            <div>
                ...
                <dd class="font-medium text-gray-900 mt-0.5">{{ $expense->vendor }}</dd>
            </div>
            <div>
                <dt class="text-gray-500">Amount</dt>
                <dd class="font-medium text-gray-900 mt-0.5">${{ number_format($expense->amount, 2) }}</dd>
            </div>
            @if ($expense->description)
            <div class="sm:col-span-2">
                <dt class="text-gray-500">Description</dt>
                <dd class="font-medium text-gray-900 mt-0.5">{{ $expense->description }}</dd>
            </div>
            @endif
            <div>
                <dt class="text-gray-500">Payment Method</dt>
                <dd class="font-medium text-gray-900 mt-0.5">{{ $expense->paymentMethodLabel() ?: '—' }}</dd>
            </div>
            @if ($expense->reference_number)
            <div>
                <dt class="text-gray-500">Reference #</dt>
                <dd class="font-medium text-gray-900 mt-0.5 font-mono">{{ $expense->reference_number }}</dd>
            </div>
            @endif
            <div>
                <dt class="text-gray-500">Recorded By</dt>
                <dd class="font-medium text-gray-900 mt-0.5">{{ $expense->creator?->name ?? '—' }}</dd>
            </div>
            <div>
                <dt class="text-gray-500">Created</dt>
                <dd class="font-medium text-gray-900 mt-0.5">{{ $expense->created_at->format('M j, Y g:i A') }}</dd>
            </div>
        </dl>
    </div>
