#!/bin/bash
# Create a clean local database for Laravel migrations
set -euo pipefail
...
if [[ -z "$DB_PASS" ]]; then
	...
	echo "Example: DB_PASS='your-strong-password' bash scripts/setup_local_mysql_fresh_db.sh" >&2
	exit 1
fi
